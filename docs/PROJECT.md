...
- **BFF**：Express 4，挂载在 `/api` 下；未命中的路径可代理到远端后端（见下文）。
...
└── server/                   # BFF workspace
    ...
    │   ├── app.ts            # Express 应用组装、/api 下 hello 与总代理
    │   ├── constants/        # 如 admin 相关常量
    │   ├── lib/              # admin 上游请求、handlers、代理工厂等
    │   ├── middleware/       # 如 admin 鉴权中间件
    │   ├── routers/          # 按文件扫描挂载到 /api/<segment>
    │   └── utils/            # registerApiRouters、mountApiSegments、parseCookieHeader 等
    └── .env.example
```
...
3. **Admin 模块**（`routers/admin.ts`）
   - 对外前缀：`/api/admin`。
   - 由 `constants/`、`lib/`、`middleware/`、`utils/` 拆分实现；路由文件仅负责挂载路径与中间件。
   - 典型路径：
     ...
     - 其余 `/api/admin/*`：先校验 Cookie 中 token，再代理到远端 `/admin/*`（并注入 `Authorization: Bearer`）。

4. **兜底代理**
   - 在 `registerRoutersFromDir` 之后，`app.ts` 中为未匹配的 `/api` 请求配置 `http-proxy-middleware`，将路径重写到远端（是否带 `/api` 前缀由 `BACKEND_STRIP_API_PREFIX` 控制）。
...
| `BACKEND_STRIP_API_PREFIX` | 为 `true` 时，兜底代理不向路径前加 `/api` |
